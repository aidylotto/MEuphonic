...
def render_to_midi(structure, tempo_bpm: int, output_path: str, mood) -> str:
    try:
        ...
        for bar_i, chord_name in enumerate(all_bar_chords):
            ...
            # release all notes after bar duration
            for i, n in enumerate(notes):
                chord_track.append(
                    Message(
                        "note_off",
        ...
        if style.guitar:
            ...
            for bar_i, chord_name in enumerate(all_bar_chords):
                ...
                for note, beat in events:
                    ...
                    delta = max(0, event_tick - last_tick)
        ...
        print("MIDI FAILED — FALLBACK MODE")
        print(e)
        ...
        for chord in ["Am", "F", "C", "G"]:
            ...
            for n in notes:
                track.append(Message("note_on", note=n, velocity=60, time=0))
            for n in notes:
                track.append(Message("note_off", note=n, velocity=0, time=mid.ticks_per_beat * 4))
